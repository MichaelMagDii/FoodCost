...
namespace Food_Cost
{
    ...
    public partial class All_Purchase_Orders : Window
    {
        ...
        public All_Purchase_Orders(PurchaseOrder _purchaseOrder)
        {
            ...
            {
                con.Open();
                string s = string.Format("select PO_Serial AS Serial, PO_NO as Number,(Select Name From Setup_Restaurant Where Code=Ship_ToRestaurant) AS Resturant, (Select Name From Setup_Kitchens Where Code=PO.Ship_ToKitchen AND Setup_Kitchens.RestaurantID=PO.Ship_ToRestaurant) AS Kitchen, (Select Name From Vendors Where Code=Vendor_ID) as Vendor,Delivery_Date as Delivery_Date FROM PO where Status <> 'Post'");
                ...
                if (grid != null && grid.SelectedItems != null && grid.SelectedItems.Count == 1)
                {
                    ...
                    if (main.GridMain.Children[0].GetType().Name == "PurchaseOrder")
                    {
                        try
                        {
                            ...
                            con.Open();
                            SqlCommand cmd = new SqlCommand(string.Format("select PO_Serial,PO_No,(select Name from Setup_Restaurant where Setup_Restaurant.Code = PO.Ship_ToRestaurant) as Restaurant_Trans_ID, (select Name from Setup_Kitchens where  Setup_Kitchens.Code = PO.Ship_ToKitchen AND Setup_Kitchens.RestaurantID=PO.Ship_ToRestaurant) as Kitchen_Trans_ID, (select Name from Vendors where Vendors.Code = Vendor_ID) as Vendor_ID,Delivery_Date,Comment,Status from PO where PO_Serial = '{0}'", dt.Rows[PO_DGV.SelectedIndex]["Serial"]), con);
                            ...
                            purchaseOrder.KitchenShipTo.Text = reader["Kitchen_Trans_ID"].ToString();
                    ...
                    else if (main.GridMain.Children[0].GetType().Name == "RecieveOrder")
                    {
                        ...
                        float from_rest_Qty = 0; float from_rest_Cost = 0; float to_rest_Qty = 0;   float to_rest_Cost = 0; float FinallQty = 0;
                        if (recieveOrder.TabControl.SelectedIndex == 1)
                        {
                            ...
                            {
                                con.Open();
                                string M = "SELECT Item_ID,Qty,Cost,Unit,Recipes FROM Requests_Items where Request_ID=" + CodeofRO;
                                ...
                                while (reader.Read())
                                {
                                    ...
                                    if ((bool)reader["Recipes"] == true)
                                    {
                                        try
                                        {
                                            DataTable TheValues = Classes.RetriveCostAndQtyRecipes(recieveOrder.FromRestaurant_Resturanttxt.Text, recieveOrder.FromKitchen_Resturanttxt.Text, reader["Item_ID"].ToString());
                                            try { from_rest_Qty = (float.Parse(TheValues.Rows[0][0].ToString())); }
                                            catch { from_rest_Qty = 0; }
                                            try { from_rest_Cost = float.Parse(TheValues.Rows[0][1].ToString()); }
                                            catch { from_rest_Cost = 0; }

                                            TheValues = Classes.RetriveCostAndQtyRecipes(recieveOrder.ToResturant_Restauranttxt.Text, recieveOrder.ToKitchen_Restauranttxt.Text, reader["Item_ID"].ToString());
                                            try { to_rest_Qty = float.Parse(TheValues.Rows[0][0].ToString()); }
                                            catch { to_rest_Qty = 0; }
                                            FinallQty = to_rest_Qty + float.Parse(reader["Qty"].ToString());
                                            //to_rest_Qty += float.Parse(reader["Qty"].ToString());
                                            try { to_rest_Cost = float.Parse(TheValues.Rows[0][1].ToString()); }
                                            catch { to_rest_Cost = 0; }
                                            to_rest_Cost = (((float.Parse(reader["Qty"].ToString()) * float.Parse(reader["Cost"].ToString())) + (to_rest_Qty * to_rest_Cost)) / FinallQty);
                                        }
                                        catch (Exception ex)
                                        { MessageBox.Show(ex.ToString()); }
                                        finally { con2.Close(); }
                                        ...
                                        double NetCostTo = FinallQty * to_rest_Cost;
                                        try
                                        {
                                            con2.Open();
                                            string S = "SELECT Name,Name2 FROM Setup_Recipes where Code='" + reader["Item_ID"].ToString() + "'";
                                            SqlCommand cmd2 = new SqlCommand(S, con2);
                                            SqlDataReader reader2 = cmd2.ExecuteReader();
                                            while (reader2.Read())
                                            {
                                                Dat.Rows.Add(IfItemRecieved, reader["Item_ID"], reader2["Name"], reader2["Name2"], reader["Unit"], false, reader["Qty"], from_rest_Qty, from_rest_Cost, NetCostFrom, FinallQty, to_rest_Cost, NetCostTo,reader["Recipes"]);
                                            }
                                            reader2.Close();
                                        }
                                        catch (Exception ex)
                                    ...
                                    else
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            while (reader2.Read())
                                            {
                                                Dat.Rows.Add(IfItemRecieved, reader["Item_ID"], reader2["Name"], reader2["Name2"], reader["Unit"], reader2["ExpDate"], reader["Qty"], from_rest_Qty, from_rest_Cost, NetCostFrom, FinallQty, to_rest_Cost, NetCostTo,reader["Recipes"]);
                                            }
                                            reader2.Close();
                                        }
                                        catch (Exception ex)
                                ...
                                while (reader.Read())
                                {
                                    if ((bool)reader["Recipes"] == true)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            DataTable TheVales = Classes.RetriveCostAndQtyRecipes(recieveOrder.Resturant_Kitchentxt.Text, recieveOrder.FromKitchen_Kitchentxt.Text, reader["Item_ID"].ToString());
                                            try { from_rest_Qty = (float.Parse(TheVales.Rows[0][0].ToString())); }
                                            catch { from_rest_Qty = 0; }
                                            try { from_rest_Cost = float.Parse(TheVales.Rows[0][1].ToString()); }
                                            catch { from_rest_Cost = 0; }

                                            TheVales = Classes.RetriveCostAndQtyRecipes(recieveOrder.Resturant_Kitchentxt.Text, recieveOrder.ToKitchen_Kitchentxt.Text, reader["Item_ID"].ToString());
                                            try { to_rest_Qty = float.Parse(TheVales.Rows[0][0].ToString()); }
                                            catch { to_rest_Qty = 0; }
                                            FinallQty = to_rest_Qty+ float.Parse(reader["Qty"].ToString());
                                            //to_rest_Qty += float.Parse(reader["Qty"].ToString());
                                            try { to_rest_Cost = float.Parse(TheVales.Rows[0][1].ToString()); }
                                            catch { to_rest_Cost = 0; }
                                            to_rest_Cost = (((float.Parse(reader["Qty"].ToString()) * float.Parse(reader["Cost"].ToString())) + (to_rest_Qty * to_rest_Cost)) / FinallQty);
                    ...
                    else if (main.GridMain.Children[0].GetType().Name == "Transfer_Kitchens")
                    {
                        ...
                        try
                        {
                            ...
                            reader.Close();
                            con2.Open();
                            string W = string.Format("((select Item_ID as Code,(select [Manual Code] from Setup_Items where Code= Item_ID) as Manual_Code,(select Name from Setup_Items where Code = Item_ID) as Name,(select Name2 from Setup_Items where Code = Item_ID) as Name2,Unit, Qty,Recipes from Transfer_Kitchens_Items where Transfer_ID = '{0}' and Recipes='False') UNION (select Item_ID as Code,(select CrossCode from Setup_Recipes where Code= Item_ID) as Manual_Code,(select Name from Setup_Recipes where Code = Item_ID) as Name,(select Name2 from Setup_Recipes where Code = Item_ID) as Name2,Unit, Qty,Recipes from Transfer_Kitchens_Items where Transfer_ID = '{0}' and Recipes='True'))", transferKitchen.transfer_No.Text);
                            ...
                            while (reader.Read())
                            {
                                if ((bool)reader["Recipes"] == true)
                                {
                                    try
                                    {
                                        ...
                                        DataTable TheValue = Classes.RetriveCostAndQtyRecipes(transferKitchen.Resturant.Text, transferKitchen.From_Kitchen.Text, reader["Code"].ToString());
                                        try { from_rest_Qty = float.Parse(TheValue.Rows[0][0].ToString()); }
                                        catch { from_rest_Qty = 0; }
                                        try { from_rest_Cost = float.Parse(TheValue.Rows[0][1].ToString()); }
                                        catch { from_rest_Cost = 0; }

                                        TheValue = Classes.RetriveCostAndQtyRecipes(transferKitchen.Resturant.Text, transferKitchen.To_Kitchen.Text, reader["Code"].ToString());
                                        try { to_rest_Qty = float.Parse(TheValue.Rows[0][0].ToString()); }
                                        catch { to_rest_Qty = 0; }
                                        try { to_rest_Cost = float.Parse(TheValue.Rows[0][1].ToString()); }
                                        catch { to_rest_Cost = 0; }
                                ...
                                dt.Rows.Add(reader["Code"], reader["Manual_Code"], reader["Name"], reader["Name2"],reader["Unit"], reader["Qty"], FromKitchenQty, FRomKitchenCost, FromKitchenTotal, ToKitchenQty, ToKitchenCost, ToKitchenTotal,reader["Recipes"]);
                            }
                        }
                        catch { }
                    ...
                    else if (main.GridMain.Children[0].GetType().Name == "Transfer_Resturant")
                    {
                        ...
                        try
                        {
                            ...
                            con2.Open();
                            string W = string.Format("((select Item_ID as Code,(select [Manual Code] from Setup_Items where Code= Item_ID) as Manual_Code,(select Name from Setup_Items where Code = Item_ID) as Name,(select Name2 from Setup_Items where Code = Item_ID) as Name2,Unit, Qty,Recipes from Transfer_Kitchens_Items where Transfer_ID = '{0}' and Recipes='False') UNION (select Item_ID as Code,(select CrossCode from Setup_Recipes where Code= Item_ID) as Manual_Code,(select Name from Setup_Recipes where Code = Item_ID) as Name,(select Name2 from Setup_Recipes where Code = Item_ID) as Name2,Unit, Qty,Recipes from Transfer_Kitchens_Items where Transfer_ID = '{0}' and Recipes='True'))", transfer_Resturant.transfer_No.Text);
                            cmd = new SqlCommand(W, con);
                            reader = cmd.ExecuteReader();
                            while (reader.Read())
                            {
                                ...
                                {
                                    Recipe = false;
                                    Recipe = (bool)reader["Recipes"];
                                    float from_rest_Qty = 0; float from_rest_Cost = 0; float to_rest_Qty = 0; float to_rest_Cost;
                                    if ((bool)reader["Recipes"] == true)
                                    {
                                        DataTable TheValue = Classes.RetriveCostAndQtyRecipes(transfer_Resturant.From_Resturant.Text, transfer_Resturant.From_Kitchen.Text, reader["Code"].ToString());
                                        try { from_rest_Qty = float.Parse(TheValue.Rows[0][0].ToString()); }
                                        catch { from_rest_Qty = 0; }
                                        try { from_rest_Cost = float.Parse(TheValue.Rows[0][1].ToString()); }
                                        catch { from_rest_Cost = 0; }

                                        TheValue = Classes.RetriveCostAndQtyRecipes(transfer_Resturant.ToResturant.Text, transfer_Resturant.To_Kitchen.Text, reader["Code"].ToString());
                                        try { to_rest_Qty = float.Parse(TheValue.Rows[0][0].ToString()); }
                                        catch { to_rest_Qty = 0; }
                                        try { to_rest_Cost = float.Parse(TheValue.Rows[0][1].ToString()); }
                                        catch { to_rest_Cost = 0; }
                                ...
                                dt.Rows.Add(reader["Code"], reader["Manual_Code"], reader["Name"], reader["Name2"], reader["Unit"], reader["Qty"], FromKitchenQty, FRomKitchenCost, FromKitchenTotal, ToKitchenQty, ToKitchenCost, ToKitchenTotal, Recipe);
                            }
